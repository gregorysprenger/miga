mytaxa.scan <- function(
  ...
  d <- apply(a[, -1], 2,
    function(x, y) sqrt(sum((sqrt(x) - sqrt(y))^2) / 2),
    y = a[, 1]
  )
  d.thr <- quantile(d, probs = 0.95, names = F, na.rm = TRUE)
  plot(1,
    xlim = c(0, length(d) + 1), ylim = c(0, 1), xlab = "", xaxs = "i", xaxt = "n",
  ...
  plot(1,
    t = "n", xlim = c(0, ncol(b) + 1), xaxs = "i", ylim = c(0, 1.2),
  ...
  # Regions (outliers)
  regs <- c()
  max_values <- c() # index value of max value in each column if d[j] > d.thr
  for (j in 1:ncol(b)) {
    if (d[j] > d.thr) {
      regs <- c(regs, j)
      max_values <- c(max_values, which.max(b[, j])[[1]])
    }
  }

  region_values <- rownames(b[max_values, ]) # get row name of max index value from max_values
  ...
  if (length(regs) > 0) {
    ...
    write.table(regs, paste(wintax, ".regions", sep = ""),
      col.names = F,
      row.names = F, quote = F
    )
  }
  ...
  plot(1,
    t = "n", xlim = c(0, 1), xaxs = "i", ylim = c(0, 1.2), yaxs = "i",
  ...
  plot(1,
    t = "n", bty = "n", xlim = c(0, 1),
    ylim = c(0, 1), xaxs = "i", yaxs = "i", axes = F
  )
  legend("top",
    pt.bg = all_cols, col = grey(0.3), pch = 22,
    legend = gsub(".*::", "", rownames(b)), ncol = 5, cex = 3 / 4, bty = "n"
  )

  # ::: REGIONS (OUTLIER) LEGEND
  region_cleaned_values <- gsub(".*::", "", region_values) # split at last :: and keep
  region_legend <- c()

  # Loop over each item in cleaned values and append to new list for legend (so numbers will show in legend)
  for (i in 1:length(region_cleaned_values)) {
    region_legend <- c(region_legend, paste(as.character(i), ". ", region_cleaned_values[i], " (region ", regs[i], ")", sep = ""))
  }

  legend("center", legend = region_legend, ncol = 3, cex = 3 / 4, bty = "n", title = "Regions (outliers)", title.col = "darkred")
